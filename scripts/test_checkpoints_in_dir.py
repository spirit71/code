...
DEFAULT_VERSION_DIR = PROJECT_ROOT / "logs" / "dinov2_vitb14" / "version_17"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--query-reliability", action="store_true", help="Build the QRL-BoQ aggregator.")
    parser.add_argument("--reliability-mode", choices=["learned", "entropy"], default="learned")
    parser.add_argument(
        "--reliability-gate",
        choices=["none", "direct", "centered_residual"],
        default="centered_residual",
    )
    parser.add_argument("--rel-target-mode", choices=["continuous", "rank", "top_bottom"], default="rank")
    parser.add_argument("--rel-loss-type", choices=["smooth_l1", "bce", "ranking"], default="smooth_l1")
...
def main() -> None:
    ...
    hparams.use_query_reliability = args.query_reliability
    hparams.reliability_mode = args.reliability_mode
    hparams.reliability_gate_mode = args.reliability_gate
    hparams.rel_target_mode = args.rel_target_mode
    hparams.rel_loss_type = args.rel_loss_type
