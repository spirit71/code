        ...
        for n2, m2 in m.named_modules():# 进一步遍历该 adapter 的子模块
            if 'D_fc2' in n2: # 如果子模块名称包含 'D_fc2'
                if isinstance(m2, nn.Linear):# 确认该子模块是线性层
                    nn.init.constant_(m2.weight, 0.)# 权重初始化为0 
...
#########################
# Domain-Routed Query Bank regularization
#########################

def reg_load_balance(gates: torch.Tensor) -> torch.Tensor:
    """
    gates: [B, K] softmax scores over experts.
    Encourage average routing distribution to be close to uniform.
    """
    prob_mean = gates.mean(dim=0)  # [K]
    K = prob_mean.shape[0]
    uniform = torch.full_like(prob_mean, 1.0 / K)
    return torch.sum((prob_mean - uniform) ** 2)


def reg_entropy(gates: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    """
    gates: [B, K] softmax scores over experts.
    Encourage high entropy routing (avoid premature one-hot).
    """
    log_gates = torch.log(gates + eps)
    entropy = -torch.sum(gates * log_gates, dim=-1)  # [B]
    # we want to maximize entropy -> add negative entropy as a loss term
    return -entropy.mean()


# reasonable default weights (can be tuned)
alpha_lb = 0.1   # load balancing strength
alpha_ent = 0.01 # entropy regularization strength
...
#### Training loop
Batch_size = DataLoader(dataset=train_dataset, **train_loader_config)
print(len(Batch_size))  #869   #62514个地点/72个地点
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=len(Batch_size)*3, gamma=0.7, last_epoch=-1)
for epoch_num in range(start_epoch_num, args.epochs_num):
    ...
    model = model.train()
    epoch_losses=[]
    for images, place_id in tqdm(Batch_size):       
        BS, N, ch, h, w = images.shape   #每一个“地点（Place ID）=4张图片”为一个批次（Batch）
        # reshape places and labels
        images = images.view(BS*N, ch, h, w) #torch.Size([72, 4, 3, 224, 224])-> ([288, 3, 224, 224])
        labels = place_id.view(-1)          #place_id[[72, 4]]  #labels =[288]

        # model returns descriptors and gating scores from Domain-Routed Query Bank
        descriptors, gates = model(images.to(args.device), return_gates=True)     #backbone返回的x ([288, 4096])
        descriptors = descriptors.cuda()

        metric_loss = loss_function(descriptors, labels) # Call the loss_function we defined above

        # regularization losses for expert routing
        lb_loss = reg_load_balance(gates)
        ent_loss = reg_entropy(gates)
        loss = metric_loss + alpha_lb * lb_loss + alpha_ent * ent_loss

        del descriptors, gates
