...
def evaluate_saved_checkpoints(trainer, model, datamodule, silent=False):
    ...
    for ckpt_path in checkpoint_paths:
        ...
        trainer.validate(model=model, datamodule=datamodule, ckpt_path=str(ckpt_path), verbose=False)
...
def parse_args():
    ...
    parser.add_argument("--test-every-epoch", action="store_true", help="Save every epoch checkpoint during training, then after fit evaluate all checkpoints on val/test sets and export per-epoch results.")
