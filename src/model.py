...
class BoQModel(L.LightningModule):
    ...
    def forward(self, x, return_intermediates=False):
        """图像前向计算；分析开关关闭时保持原训练/测试接口不变。"""
        # raw_backbone 通常是 [B,C,H,W] 的二维特征图。
        raw_backbone = self.backbone(x)
        if return_intermediates:
            # QASSR 需要空间 token 和 attention，因此请求聚合器返回中间结果。
            result = self.aggregator(raw_backbone, return_intermediates=True)
            # 同时把原 backbone 网格展平为 [B,H*W,C]，便于与 x0/x1/x2 比较。
            result["raw_backbone"] = raw_backbone.flatten(2).permute(0, 2, 1)
            return result
        x, attns = self.aggregator(raw_backbone)
        return x, attns
