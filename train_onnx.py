...
from torch.onnx import export # 新增：导入 ONNX 导出函数
...
# 假设您的图像大小为 3x224x224 (这是 VPR 常见的输入)
BS, ch, h, w = 1, 3, 224, 224 # 导出的 ONNX 只需要一个虚拟的单张输入
dummy_input = torch.randn(BS, ch, h, w).to(args.device) # 确保在正确设备上
...
for epoch_num in range(start_epoch_num, args.epochs_num):
    ...
    for images, place_id in tqdm(Batch_size):       
        ...
        # 使用带 Domain-Routed Query Bank 的模型输出
        descriptors, _ = model(images.to(args.device), return_gates=True)     #backbone返回的x ([288, 4096])
...
#### 🎯 核心修改：导出模型为 ONNX 格式
    ...
    try:
        model.eval() # 切换到评估模式
        onnx_filename = f"model_epoch_{epoch_num:02d}.onnx"
        onnx_path = join(args.save_dir, onnx_filename)

        # model.module 是实际的模型实例 (从 DataParallel 中取出)
        export(
            model.module,
            dummy_input,
            onnx_path,
        ...
        logging.error(f"Failed to export model to ONNX for epoch {epoch_num:02d}: {e}")
        model.train() # 确保即使出错也切回训练模式
    # --------------------------------------------------------------------------------------
    #### 🎯 核心修改结束
    if is_best:
