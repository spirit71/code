...
#这是一个视觉Transformer类模型的特征前向函数。它接收输入图像tokens（或图像经过patch embedding后的序列）以及可选的 masks，经过一串 Transformer blocks 与并行的低秩适配器 LoPA（Low-rank Parallel Adaptation）后，输出多种规范化后的token切片：CLS token、注册token（register tokens）以及patch tokens，同时还返回若干中间结果，便于下游任务取用。
    def forward_features(self, x, masks=None):
        ...
        if isinstance(x, list):  #x.shape=torch.Size([288, 3, 224, 224])
            return self.forward_features_list(x, masks)
        #主路径专注于单张量输入
        x = self.prepare_tokens_with_masks(x, masks) #x.shape ([288, 257, 768])
        ...
        x_norm = self.norm(x)
        return {
            "x_norm_clstoken": x_norm[:, 0:1,:], #[288, 1, 768]
            "x_norm_regtokens": x_norm[:, 1 : self.num_register_tokens + 1],  #[288, 0, 768])  #num_register_tokens (1，0，768)
            "x_norm_patchtokens": x_norm[:, self.num_register_tokens + 1 :], #([288, 256, 768])
            "x_prenorm": x,
            "x_norm": x_norm,  #([288, 257, 768])
            "masks": masks,
        }
